...
if uploaded_file is not None:
    try:
        ...
        if df.shape[0] < 500 or df.shape[1] < 12:
            ...
            # --- CRITICAL FIX: TARGET CLEANING ---
            target_name = df.columns[-1]
            
            # 1. Remove any rows where the target (y) is missing
            df = df.dropna(subset=[target_name])
            
            # 2. Fill missing values for features (X)
            ...
            # 5. Fix for your Error: Convert y safely
            # Factorize converts text to numbers (0, 1, 2...) and handles residuals
            y_clean, _ = pd.factorize(y)
            y = pd.Series(y_clean) 
